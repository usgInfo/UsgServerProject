 ...
 * @author user
 */
public class AttendanceUnprocessService extends HttpServlet {

    Logger logger = Logger.getLogger(AttendanceUnprocessService.class);
    String privilege = ApplicationConstants.PV_EMPLOYEE_ATTENDANCE_CREATE;
    String privilege1 = ApplicationConstants.PV_EMPLOYEE_ATTENDANCE_UPDATE;
    String privilege2 = ApplicationConstants.PV_EMPLOYEE_ATTENDANCE_DELETE;
        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                if (authorized || authorized1 || authorized2) {
                    String attendanceJson = request.getParameter("unProcessAttendance");
                    String userId = request.getParameter("loginUserId");
                    //System.out.println("ghsdtfeyt" + attendanceJson);

                    boolean result = EmpAttendanceManager.unprocessAttendance(attendanceJson, userId);
//                String result = new EmpAttendanceManager().save(attendanceJson, month, year);
                    ...
                    logger.info(Common.getLogMsg("SaveEmpAttendanceService", ApplicationConstants.FAIL, ApplicationConstants.HTTP_STATUS_UNAUTHORIZED));
                }
            } else {
