 ...
 * @author user
 */
public class SalarySummaryReportService extends HttpServlet {

    Logger logger = Logger.getLogger(SalarySlipRegisterReportService.class);

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setHeader("Content-Disposition", "inline; filename=\"SalarySummaryDetails.pdf\"");
        response.setContentType("application/pdf;charset=UTF-8;name=\"SalarySummaryDetails.pdf\"");
        ...
        try {
            ...
            ByteArrayOutputStream baos = new SalarySummaryDetailsManager().getSearchResult(empdata, month, year, ddo, contextPath, fin);
            if (baos != null) {
                PDFMergerUtility ut = new PDFMergerUtility();
                ut.addSource(new ByteArrayInputStream(baos.toByteArray()));
                ut.setDestinationStream(out);
                ut.mergeDocuments();
                ut.setDestinationFileName("SalarySummaryDetails.pdf");
