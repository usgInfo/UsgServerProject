 ...
 * @author upendra
 */
public class BankStatementReportService extends HttpServlet {

    Logger logger = Logger.getLogger(BankStatementReportService.class);

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setHeader("Content-Disposition", "inline; filename=\"BankStatementReport.pdf\"");
        response.setContentType("application/pdf;charset=UTF-8;name=\"BankStatementReport.pdf\"");
        ...
        try {
            ...
            String bank = URLDecoder.decode(request.getParameter("bank"));
            ...
            ByteArrayOutputStream baos = new BankStatementManager().bankStatementReport(month, year, empdata, contextPath, bank,fin);
            if (baos != null) {
                PDFMergerUtility ut = new PDFMergerUtility();
                ut.addSource(new ByteArrayInputStream(baos.toByteArray()));
                ut.setDestinationStream(out);
                ut.mergeDocuments();
                ut.setDestinationFileName("BankStatementReport.pdf");
