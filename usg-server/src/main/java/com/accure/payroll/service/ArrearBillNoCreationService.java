...
import com.accure.payroll.manager.ArrearBillCreationManager;
 ...
 * @author user
 */
public class ArrearBillNoCreationService extends HttpServlet {

    Logger logger = Logger.getLogger(AttendanceUnprocessService.class);
    String privilege = ApplicationConstants.PV_ARREAR_BILL_CREATE;
        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                if (authorized) {
                    ...
                    status = new ArrearBillCreationManager().setBillNumber(empObjId);
                    status = new ArrearBillCreationManager().updateIsLockedTrue(empObjId);
                    ...
                    if (status) {
                        ...
                        //logger.info(Common.getLogMsg("SalaryBillSetBillNumberService", ApplicationConstants.DELETE, ApplicationConstants.FAIL));
                    }
                } else {
                    ...
                    logger.info(Common.getLogMsg("ArrearBillNoCreationService", ApplicationConstants.FAIL, ApplicationConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_INVALID_SESSION);
                out.write(new Gson().toJson(ApplicationConstants.HTTP_STATUS_INVALID_SESSION));
                //logger.info(Common.getLogMsg("SalaryBillSetBillNumberService", ApplicationConstants.FAIL, ApplicationConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            //logger.error(Common.getLogMsg("SalaryBillSetBillNumberService", ApplicationConstants.ERROR, stack.toString()));
