 ...
 * @author deepak2310
 */
public class BankStatementService extends HttpServlet {

    Logger logger = Logger.getLogger(BankStatementService.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                List<Employee> postingCityList = null;
                List<Employee> pfTypeList = null;
                List<FundType> fundTypeList = null;
                List<BudgetHeadMaster> budgetHeadList = null;
                List<Bank> bankNameList = null;
                FundType fundType;
                if (ApplicationConstants.DEPARTMENT.equals(action)) {
                    ...
                    if (departmentList != null) {
                        ...
                        logger.info(Common.getLogMsg("BankStatementService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.SUCCESS));
                        ...
                        logger.info(Common.getLogMsg("BankStatementService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.FAIL));
                    }
                    ...
                    pfTypeList = new SalarySlipManager().fetchAllEmployeeData();
                    if (pfTypeList != null) {
                    ...
                    postingCityList = new SalarySlipManager().fetchAllEmployeeData();
                    if (postingCityList != null) {
                        Employee employee = new Employee();
                        employee.setPostingCity(postingCityList.get(0).getPostingCity());
                ...
                } else if (ApplicationConstants.BANK_NAME.equals(action)) {
                    Set<String> bank = new SalarySlipManager().fetchAllBank();
                    if (bank != null) {
                        out.write(new Gson().toJson(new Common().onSuccess(ApplicationConstants.HTTP_STATUS_SUCCESS, ApplicationConstants.BANK_LIST, bank)));
                        request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_SUCCESS);
                        logger.info(Common.getLogMsg("BankStatementService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.SUCCESS));
                    } else if (fundTypeList == null) {
                        out.write(new Gson().toJson(new Common().LoginonFailure(ApplicationConstants.HTTP_STATUS_NODATA, ApplicationConstants.NO_DATA_FOUND)));
                        ...
                        request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(new Common().onFailure(ApplicationConstants.HTTP_STATUS_FAIL, ApplicationConstants.BANK_LIST_fAILED, bank)));
                        logger.info(Common.getLogMsg("BankStatementService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.FAIL));
                    }
                }
            } else {
            ...
            logger.error(Common.getLogMsg("BankStatementService", ApplicationConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
