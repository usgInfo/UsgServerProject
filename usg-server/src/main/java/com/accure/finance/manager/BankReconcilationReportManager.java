...
public class BankReconcilationReportManager {
   ...
   public ByteArrayOutputStream bankReconcilationReport(String fromDate, String toDate, String voucherdata, String ledger, String path, String fin, String fromDateStr, String ddo, String location, String finYear) throws DocumentException, FileNotFoundException, Exception {
        ...
        String ledNameToDisplay = "";

//        String finIdJson = DBManager.getDbConnection().fetch(ApplicationConstants.BUDGET_FINACIAL_YEAR_TABLE, finYear);
...
//            String finYearId = finIdListObj.getYear();
            
        HashMap<String, String> conditionMap = new HashMap<String, String>();
        conditionMap.put("year", finYear);
            ...
            for (PaymentVoucher payVou : payVocList) {
                ...
                for (int k1 = 0; k1 < payVou.getLedgerList().size(); k1++) {
                    ...
                    if ((entryStatusRpt1 != null)) {
                        String abc = (payVou.getLedgerList().get(k1).getLedger());
                        String abc1 = (payVou.getLedgerList().get(k1).getLedgerName());
//                        if ((payVou.getLedgerList().get(k1).getLedger().equals(ledger)) && (payVou.getPaymentMode().equals("Cheque")) && (payVou.getLedgerList().get(k1).getDrCr().equals("Cr")) && (payVou.getEntryStatus().equals("Uncleared Entries")) && (vdms >= fromVD) && (vdms <= toVD) && ((payVou.getPostingStatus().equals("Posted")))) {
                         if ((payVou.getLedgerList().get(k1).getLedger().equals(ledger)) && (payVou.getPaymentMode().equals("Cheque")) && (payVou.getLedgerList().get(k1).getDrCr().equals("Cr")) &&  (vdms >= finStart) && (vdms <= toVD) && ((payVou.getPostingStatus().equals("Posted")))&& ((payVou.getEntryStatus().equals("Uncleared Entries")))) {
//                            if(payVou.getPaymentMode() == "Cheque"){
                             for(int ol = 0; ol < payVou.getLedgerList().size(); ol++){
                            if(payVou.getLedgerList().get(ol).getDrCr().equals("Dr")){
                                ledNameToDisplay = payVou.getLedgerList().get(ol).getLedgerName();
                                    }
                            }
                                for (int cn = 0; cn < payVou.getChequeList().size(); cn++) {
                                String paymentCheckAmount = (payVou.getChequeList().get(cn).getChequeAmount());
                                double paymentCheckAmountDouble = Double.parseDouble(paymentCheckAmount);
                                if(vdms < toVD){
                                openingBalance = openingBalance + paymentCheckAmountDouble;
                                }
//                                if((payVou.getEntryStatus().equals("Uncleared Entries"))){
//                                    openingBalance = openingBalance + paymentCheckAmountDouble;
//                                }
//                                if((payVou.getEntryStatus().equals("Cleared Entries"))){
//                                    openingBalance = openingBalance - paymentCheckAmountDouble;
//                                }
...
//                            payVou.getLedgerList().get(cn).setDrCr(payVou.getLedgerList().get(k1).getDrCr());
                                ...
                                payVou.getLedgerList().get(cn).setLedgerName(ledNameToDisplay);
...
//                        if ((recVou.getLedgerList().get(k3).getLedger().equals(ledger)) && (recVou.getPaymentMode().equals("Cheque")) && (recVou.getLedgerList().get(k3).getDrCr().equals("Dr")) && (recVou.getEntryStatus().equals("Uncleared Entries")) && (vdms >= finStart) && (vdms <= fromVD) && ((recVou.getPostingStatus().equals("Posted")))) {
                        if ((recVou.getLedgerList().get(k3).getLedger().equals(ledger)) && (recVou.getPaymentMode().equals("Cheque")) && (recVou.getLedgerList().get(k3).getDrCr().equals("Dr")) &&  (vdms >= finStart) && (vdms <= toVD) && ((recVou.getPostingStatus().equals("Posted")))&& ((recVou.getEntryStatus().equals("Uncleared Entries")))) {
                            for(int ol = 0; ol < recVou.getLedgerList().size(); ol++){
                            if(recVou.getLedgerList().get(ol).getDrCr().equals("Cr")){
                                ledNameToDisplay = recVou.getLedgerList().get(ol).getLedgerName();
                            }
                        }
                            for (int cn = 0; cn < recVou.getChequeList().size(); cn++) {
                                
                                String receiptCheckAmount = (recVou.getChequeList().get(cn).getChequeAmount());
                                double receiptCheckAmountDouble = Double.parseDouble(receiptCheckAmount);
                                if(vdms < toVD){
                                openingBalance = openingBalance - receiptCheckAmountDouble;
                                }
//                                if((recVou.getEntryStatus().equals("Uncleared Entries"))){
//                                    openingBalance = openingBalance + receiptCheckAmountDouble;
//                                }
//                                if((recVou.getEntryStatus().equals("Cleared Entries"))){
//                                    openingBalance = openingBalance - receiptCheckAmountDouble;
//                                }
...
//                            recVou.getLedgerList().get(k3).setChequeAmtReport(recVou.getChequeAmount());
                                ...
                                recVou.getLedgerList().get(cn).setLedgerName(ledNameToDisplay);
...
//                        if ((conVou.getLedgerList().get(k2).getLedger().equals(ledger)) && (conVou.getPaymentMode().equals("Cheque")) && (conVou.getEntryStatus().equals("Uncleared Entries")) && (vdms >= finStart) && (vdms <= fromVD) && ((conVou.getPostingStatus().equals("Posted")))) {
                        if ((conVou.getLedgerList().get(k2).getLedger().equals(ledger)) && (conVou.getPaymentMode().equals("Cheque")) && (vdms >= finStart) && (vdms <= toVD) && ((conVou.getPostingStatus().equals("Posted")))&& ((conVou.getEntryStatus().equals("Uncleared Entries")))) {
                            for(int ol = 0; ol < conVou.getLedgerList().size(); ol++){
                            if(conVou.getLedgerList().get(k2).getDrCr().equals("Dr")){
                                if(conVou.getLedgerList().get(ol).getDrCr().equals("Cr")){
                                    ledNameToDisplay = conVou.getLedgerList().get(ol).getLedgerName();
                                }
                            }
                            if(conVou.getLedgerList().get(k2).getDrCr().equals("Cr")){
                                if(conVou.getLedgerList().get(ol).getDrCr().equals("Dr")){
                                    ledNameToDisplay = conVou.getLedgerList().get(ol).getLedgerName();
                                }
                            }
                            ...
                            for (int cn = 0; cn < conVou.getChequeList().size(); cn++) {
                                String contraCheckAmount = (conVou.getChequeList().get(cn).getChequeAmount());
                                double contraCheckAmountDouble = Double.parseDouble(contraCheckAmount);
                                if((conVou.getLedgerList().get(k2).getDrCr().equals("Dr")) && vdms < toVD){
                                    openingBalance = openingBalance - contraCheckAmountDouble;
                                }
                                if((conVou.getLedgerList().get(k2).getDrCr().equals("Cr")) && vdms < toVD){
                                    openingBalance = openingBalance + contraCheckAmountDouble;
                                }
//                                if((conVou.getEntryStatus().equals("Uncleared Entries"))){
//                                    openingBalance = openingBalance + contraCheckAmountDouble;
//                                }
//                                if((conVou.getEntryStatus().equals("Cleared Entries"))){
//                                    openingBalance = openingBalance - contraCheckAmountDouble;
//                                }
...
//                            conVou.getLedgerList().get(k2).setChequeAmtReport(conVou.getChequeAmount());
                                ...
                                conVou.getLedgerList().get(cn).setLedgerName(ledNameToDisplay);
...
//financial year
        ...
        PdfPTable tableLedName = new PdfPTable(1);
        tableLedName.setWidthPercentage(100);
        tableLedName.setWidths(onecolumnwidth);

        PdfPCell tableLedNamecella = null;
        if (ledger != null) {
            String ledJson = DBManager.getDbConnection().fetch(ApplicationConstants.LEDGER_TABLE, ledger);

            List<Ledger> ledgerList = new Gson().fromJson(ledJson, new TypeToken<List<Ledger>>() {
            }.getType());
            Ledger led = ledgerList.get(0);
            String ledgerNamePrint = new CashBookReportManager().getLedgerCodeAndLedgerName(((LinkedTreeMap<String, String>) led.getId()).get("$oid"), led.getLedgerName());

//            lLobj.setLedgerName(ledgerName);
            tableLedNamecella = new PdfPCell(new Paragraph("Ledger Name :" + led.getLedgerName(), font2));
            tableLedNamecella.setBorderColor(BaseColor.BLACK);
            tableLedNamecella.setBackgroundColor(BaseColor.WHITE);
            tableLedNamecella.setHorizontalAlignment(Element.ALIGN_CENTER);
            tableLedNamecella.setVerticalAlignment(Element.ALIGN_MIDDLE);
            tableLedNamecella.setBorder(Rectangle.NO_BORDER);
        }
        tableLedName.addCell(tableLedNamecella);
        document.add(tableLedName);
        }
...
//                    table2cell6 = new PdfPCell(new Paragraph(ledNameToDisplay, font3));
                    table2cell6 = new PdfPCell(new Paragraph(merged.get(i).getLedgerName(), font3));
...
//                }
                if (i > 0) {
                    ...
                    if (drcrs.equalsIgnoreCase(drcrs1)) {
                        drCrText = merged.get(i - 1).getDrCr();
//                        table2cell9 = new PdfPCell(new Paragraph(chequeAmountDouble + " " + drCrText, font3));
                        table2cell9 = new PdfPCell(new Paragraph(chequeAmountDouble + " ", font3));
                    } else {
//                        table2cell9 = new PdfPCell(new Paragraph(chequeAmountDouble + " " + drCrText, font3));
                        table2cell9 = new PdfPCell(new Paragraph(chequeAmountDouble + " ", font3));
                    }
