 ...
 * @author accure
 */
public class LedgerBookReportManager {

    public ByteArrayOutputStream ledgerBookPdfStatement(String date, String voucherdata, String ledger, String path, String fin) throws DocumentException, FileNotFoundException, Exception {
...
//            openingBalance = 0.00;
                    ...
                    if ((payVou.getLedgerList().get(k1).getLedger()) != null) {
                        if (((payVou.getLedgerList().get(k1).getLedger()).equals(ledger)) && ((payVou.getPostingStatus().equals("Posted")))) {
                            ...
                            for (l1 = 0; l1 < payVou.getLedgerList().size(); l1++) {
                                if ((payVou.getLedgerList().get(l1).getDrCr()).equals("Cr")) {
        ...
        if (date != null && date != "" && !date.isEmpty()) {
            ...
            journalSearchQuery = journalSearchQuery + " and jv.voucherDateInMilliSecond=" + dateVD;
            ...
            for (JournalVoucher jouVou : jouVocList) {
                for (k2 = 0; k2 < jouVou.getLedgerList().size(); k2++) {
                    if ((jouVou.getLedgerList().get(k2).getLedger()) != null) {
                        if (((jouVou.getLedgerList().get(k2).getLedger()).equals(ledger)) && ((jouVou.getPostingStatus().equals("Posted"))) && ((jouVou.getLedgerList().get(k2).getDrCr()).equals("Cr"))) {

                            for (l2 = 0; l2 < jouVou.getLedgerList().size(); l2++) {
                                if ((jouVou.getLedgerList().get(l2).getDrCr()).equals("Dr")) {
                        ...
                        if (((jouVou.getLedgerList().get(k2).getLedger()).equals(ledger)) && ((jouVou.getPostingStatus().equals("Posted"))) && ((jouVou.getLedgerList().get(k2).getDrCr()).equals("Dr"))) {
...
//financial year
        ...
        LedgerBookReportManager.RoundRectangle roundRectangle = new LedgerBookReportManager.RoundRectangle();
        ...
        PdfPCell cell = new PdfPCell(new Phrase("Ledger Book", font1));
        cell.setColspan(4);
        cell.setRowspan(5);
        ...
        if (lLobj.getLedger() != null) {
            String gaJson = DBManager.getDbConnection().fetch(ApplicationConstants.LEDGER_TABLE, lLobj.getLedger());

            List<Ledger> gaList = new Gson().fromJson(gaJson, new TypeToken<List<Ledger>>() {
            }.getType());
            Ledger gal = gaList.get(0);
            String ledgerName = new CashBookReportManager().getLedgerCodeAndLedgerName(((LinkedTreeMap<String, String>) gal.getId()).get("$oid"), gal.getLedgerName());
...
//            table6cell1.setBorder(Rectangle.NO_BORDER);
                ...
                if (merged.get(i).getVoucherDtReport() != null) {
...
//                PdfPCell table6cell3 = new PdfPCell(new Paragraph(lt.getLedgerCategory(), font3));
...
//            table6cell3.setBorder(Rectangle.NO_BORDER);
                ...
                PdfPCell table6cell4 = null;
                if (merged.get(i).getShortNarration() != null) {
                    table6cell4 = new PdfPCell(new Paragraph(merged.get(i).getShortNarration(), font3));
...
//            table6cell4.setBorder(Rectangle.NO_BORDER);
                ...
                PdfPCell table6cell5 = null;
                if (merged.get(i).getShortNarration() != null) {
                    table6cell5 = new PdfPCell(new Paragraph(merged.get(i).getVoucherNoReport(), font3));
...
//            table6cell5.setBorder(Rectangle.NO_BORDER);
                ...
                PdfPCell table6cell6 = null;
                if (merged.get(i).getShortNarration() != null) {
                    table6cell6 = new PdfPCell(new Paragraph(merged.get(i).getVoucherTyReport(), font3));
...
//            table6cell6.setBorder(Rectangle.NO_BORDER);
                ...
                double value = 0.00;
                if (merged.get(i).getDrAmount() != null) {
...
//            table6cell7.setBorder(Rectangle.NO_BORDER);
                ...
                double value1 = 0.00;
                if (merged.get(i).getCrAmount() != null) {
