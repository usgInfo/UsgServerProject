 ...
 * @author KC
 */
public class LearningCentreDeleteService extends HttpServlet {
Logger logger = Logger.getLogger(LearningCentreDeleteService.class);
    /**
...
//                boolean authorized = UserManager.checkUserPrivilege(currentUser, privilege);
                boolean authorized = true;
                    ...
                    String status = new LearningCentreManager().deleteLearningCentre(id, userId);
                    ...
                    if (status.equalsIgnoreCase(ApplicationConstants.SUCCESS)) {
                        ...
                        logger.info(Common.getLogMsg("LearningCentreDeleteService", ApplicationConstants.DELETE, ApplicationConstants.FAIL));
                    ...
                    logger.info(Common.getLogMsg("LearningCentreDeleteService", ApplicationConstants.FAIL, ApplicationConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_INVALID_SESSION);
                out.write(new Gson().toJson(ApplicationConstants.HTTP_STATUS_INVALID_SESSION));
                logger.info(Common.getLogMsg("LearningCentreDeleteService", ApplicationConstants.FAIL, ApplicationConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("LearningCentreDeleteService", ApplicationConstants.ERROR, stack.toString()));
