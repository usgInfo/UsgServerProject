 ...
 * @author KC
 */
public class LearningCentreUpdateService extends HttpServlet {
Logger logger = Logger.getLogger(LearningCentreUpdateService.class);
    /**
...
//                boolean authorized = UserManager.checkUserPrivilege(currentUser, privilege);
                if (true) {
                    ...
                    String learningCentreId = request.getParameter("id");

                    Type type = new TypeToken<LearningCenter>() {
                    }.getType();
                    LearningCenter learningCentre = new Gson().fromJson(learningCentreJson, type);

                    String status = new LearningCentreManager().updateLearningCentre(learningCentre, learningCentreId, userId);
...
//                        out.write(new Gson().toJson(new Common().onSuccess(ApplicationConstants.HTTP_STATUS_SUCCESS, ApplicationConstants.LOCATION_INFO_UPDATED, status)));
                        ...
                        logger.info(Common.getLogMsg("LearningCentreUpdateService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.SUCCESS));
                    } else if (status.equalsIgnoreCase(ApplicationConstants.SUCCESS)) {
                        request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(status));
                        logger.info(Common.getLogMsg("LearningCentreUpdateService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.FAIL));
            ...
            logger.error(Common.getLogMsg("LearningCentreUpdateService", ApplicationConstants.ERROR, stack.toString()));
