 ...
 * @author KC
 */
public class CollegeMasterSaveService extends HttpServlet {

    Logger logger = Logger.getLogger(CollegeMasterSaveService.class);
    /**
...
//                boolean authorized = UserManager.checkUserPrivilege(currentUser, privilege);
                boolean authorized = true;
                 if (authorized) {
                    ...
                    String result = new CollegeManager().createCollege(collegeName, loginId);
            ...
            out.write(new Gson().toJson(new Common().onFailure(ApplicationConstants.HTTP_STATUS_EXCEPTION, ApplicationConstants.COLLEGE_CREATION_FAILED, "")));
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("CollegeMasterSaveService", ApplicationConstants.ERROR, stack.toString()));
        } finally {
            out.close();
        }
    }
