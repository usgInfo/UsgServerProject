 ...
 * @author accure
 */
public class EaditNatureService extends HttpServlet {

    org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(UpdateReligionService.class);
    String privilege = ApplicationConstants.PV_UPDATE_NATURE;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                String result = new NatureManager().update(id, name,loginUserId);

               if (result.equalsIgnoreCase(ApplicationConstants.SUCCESS)) {
                        request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_SUCCESS);
                    out.write(new Gson().toJson(result));
                    logger.info(Common.getLogMsg("EaditNatureService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.SUCCESS));
                    ...
                    logger.info(Common.getLogMsg("EaditNatureService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.FAIL));
                ...
                logger.info(Common.getLogMsg("EaditNatureService", ApplicationConstants.FAIL, ApplicationConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("EaditNatureService", ApplicationConstants.ERROR, stack.toString()));
