 ...
 * @author Asif
 */
public class SalaryHeadUpdateService extends HttpServlet {

    Logger logger = Logger.getLogger(SalaryHeadUpdateService.class);
    String privilege = ApplicationConstants.PV_UPDATE_SALARY_HEAD;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                if (authorized) 
                {
                    String salaryheadJson = request.getParameter("salaryheadJson");
                    Type type = new TypeToken<SalaryHead>() {
                    }.getType();
                    SalaryHead salaryhead = new Gson().fromJson(salaryheadJson, type);
                    String salaryheadId = request.getParameter("salaryheadId");
                    String userId = request.getParameter("userId");
                String status = new SalaryHeadManager().update(salaryhead, salaryheadId, userId);
                 if (status.equalsIgnoreCase(ApplicationConstants.SUCCESS)) {
                        ...
                        logger.info(Common.getLogMsg("SalaryHeadUpdateService", ApplicationConstants.UPDATE, ApplicationConstants.FAIL));
                    ...
                    } else 
                    {
                        ...
                        logger.info(Common.getLogMsg("SalaryHeadUpdateService", ApplicationConstants.FAIL, ApplicationConstants.INVALID_SESSION));
                    }
                } else {
                    ...
                    out.write(new Gson().toJson(new Common().onFailure(ApplicationConstants.HTTP_STATUS_UNAUTHORIZED, "Unauthorized access", null)));
                        logger.info(Common.getLogMsg("SalaryHeadUpdateService", ApplicationConstants.FAIL, ApplicationConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_INVALID_SESSION);
                out.write(new Gson().toJson(ApplicationConstants.HTTP_STATUS_INVALID_SESSION));
            ...
            logger.error(Common.getLogMsg("SalaryHeadUpdateService", ApplicationConstants.ERROR, stack.toString()));
