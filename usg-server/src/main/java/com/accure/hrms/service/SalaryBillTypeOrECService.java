 ...
 * @author user
 */
public class SalaryBillTypeOrECService extends HttpServlet {
    
    Logger logger = Logger.getLogger(SalaryBillTypeOrECService.class);
    String privilege = ApplicationConstants.PV_SALARY_BILL_TYPE_CREATE;
        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                if (authorized) {
                    ...
                    SalaryBillTypeOREmployeeCategory sbt = new Gson().fromJson(objJson, type);
                    
                    String result = new SalaryBillTypeOrEmployeeCategoryManager().save(sbt, loginUserId);
                    if (result.equalsIgnoreCase(ApplicationConstants.SUCCESS)) {
                        request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_SUCCESS);
                        out.write(new Gson().toJson(result));
                        logger.info(Common.getLogMsg("SalaryBillTypeOrECService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.SUCCESS));
                        ...
                        logger.info(Common.getLogMsg("SalaryBillTypeOrECService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.FAIL));
                    ...
                    logger.info(Common.getLogMsg("SalaryBillTypeOrECService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
                request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_INVALID_SESSION);
                out.write(new Gson().toJson(ApplicationConstants.HTTP_STATUS_INVALID_SESSION));
                logger.info(Common.getLogMsg("SalaryBillTypeOrECService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            ...
            logger.info(Common.getLogMsg("SalaryBillTypeOrECService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.HTTP_STATUS_EXCEPTION));
        } finally {
            out.close();
        }
    }
