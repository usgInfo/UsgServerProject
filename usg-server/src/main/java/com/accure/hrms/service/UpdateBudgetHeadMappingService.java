 ...
 * @author accure
 */
public class UpdateBudgetHeadMappingService extends HttpServlet {

    Logger logger = Logger.getLogger(BudgetHeadMappingService.class);
    String privilege = ApplicationConstants.PV_UPDATE_BUDGET_HEAD;
        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                if (authorized) {
                    ...
                    String update = request.getParameter("uJson");
                    Type type = new TypeToken<BudgetHeadMaster>() {
                    }.getType();
                    BudgetHeadMaster bhm = new Gson().fromJson(update, type);
                    String id = request.getParameter("id");
                    String userid = request.getParameter("userid");
                    String result = new BudgetHeadMappingManager().update(bhm, id, userid);
                    if (result.equalsIgnoreCase(ApplicationConstants.SUCCESS)) {
                        request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_SUCCESS);
                        out.write(new Gson().toJson(result));
                        logger.info(Common.getLogMsg("UpdateBudgetHeadMappingService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.SUCCESS));
                        ...
                        logger.info(Common.getLogMsg("UpdateBudgetHeadMappingService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.FAIL));
