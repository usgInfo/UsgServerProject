 ...
 * @author accure
 */
public class GazettedService extends HttpServlet {

    org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(GazettedService.class);
    String privilege = ApplicationConstants.PV_CREATE_GAD_NONGAD;
        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                if (authorized) {
                    ...
                    String result = new GAManager().saveGa(json, loginUserId);

               if (result.equalsIgnoreCase(ApplicationConstants.SUCCESS)) {
                        request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_SUCCESS);
                        out.write(new Gson().toJson(result));
                    logger.info(Common.getLogMsg("GazettedService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.SUCCESS));
                    ...
                    logger.info(Common.getLogMsg("GazettedService", ApplicationConstants.AUTHENTICATION, ApplicationConstants.FAIL));
                ...
                logger.info(Common.getLogMsg("GazettedService", ApplicationConstants.FAIL, ApplicationConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("GazettedService", ApplicationConstants.ERROR, stack.toString()));
