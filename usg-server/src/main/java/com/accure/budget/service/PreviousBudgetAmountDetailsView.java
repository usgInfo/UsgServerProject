        ...
        try {
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                if (resultJson != null && !resultJson.isEmpty()) {
                    ...
                    logger.info(Common.getLogMsg("PreviousBudgetAmountDetailsView", ApplicationConstants.VIEW, ApplicationConstants.SUCCESS));
                } else {
                    request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_FAIL);
                    out.write(new Gson().toJson(ApplicationConstants.HTTP_STATUS_FAIL));
                    logger.info(Common.getLogMsg("PreviousBudgetAmountDetailsView", ApplicationConstants.VIEW, ApplicationConstants.FAIL));
                }

            } else {
                request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_INVALID_SESSION);
                out.write(new Gson().toJson(ApplicationConstants.HTTP_STATUS_INVALID_SESSION));
                logger.info(Common.getLogMsg("PreviousBudgetAmountDetailsView", ApplicationConstants.FAIL, ApplicationConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_EXCEPTION);
            StringWriter stack = new StringWriter();
            ex.printStackTrace(new PrintWriter(stack));
            logger.error(Common.getLogMsg("PreviousBudgetAmountDetailsView", ApplicationConstants.ERROR, stack.toString()));
