 ...
 * @author user
 */
public class SanctionIncomeSubmitService extends HttpServlet {

    Logger logger = Logger.getLogger(SanctionIncomeSubmitService.class);
    String privilege = ApplicationConstants.PV_INCOME_BUDGET_UNIVERSITY_CREATE;
    String privilege1 = ApplicationConstants.PV_INCOME_BUDGET_UNIVERSITY_UPDATE;
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                if (authorized || authorized1) {
                    ...
                    for (Iterator<SanctionUniversityIncomeBudget> iterator = list.iterator(); iterator.hasNext();) {
                        ...
                        String id = next.getConsolidateExpenseBudgetId();
                        //System.out.println("id---------------> " + id);
                        if (new SanctionUniversityIncomeBudgetManager().SubmitData(id, loginUserId)) {
                            resultCount++;
                        }
                    }
                    String result = "false";
...
//                String result = new SactionUniversityExpenseManager().save(fc, loginUserId);
                    ...
                    logger.info(Common.getLogMsg("SanctionIncomeSubmitService", ApplicationConstants.FAIL, ApplicationConstants.UNAUTHORIZED_ACCESS));
                }
            } else {
