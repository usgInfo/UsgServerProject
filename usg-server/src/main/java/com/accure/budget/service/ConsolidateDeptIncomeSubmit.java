 ...
 * @author accure
 */
public class ConsolidateDeptIncomeSubmit extends HttpServlet {

    Logger logger = Logger.getLogger(ConsolidateDeptIncomeSubmit.class);
            ...
            if (SessionManager.checkUserSession(session)) {
                ...
                User currentUser = (User) session.getAttribute("user");
                    ...
                    for (Iterator<ConsolidateDepartmentIncome> iterator = list.iterator(); iterator.hasNext();) {
                        ...
                        if (new ConsolidateDeptIncomeManager().SubmitData(id, loginUserId)) {
                            resultCount++;
                        }
                    }
                    String result = "false";
...
//                String result = new ConsolidateIncomeBudgetManager().save(fc, loginUserId);
                    if (result != null && !result.isEmpty()) {
                        ...
                        logger.info(Common.getLogMsg("ConsolidateDeptIncomeSubmit", ApplicationConstants.AUTHENTICATION, ApplicationConstants.SUCCESS));
                    } else {
                        request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_FAIL);
                        out.write(new Gson().toJson(ApplicationConstants.HTTP_STATUS_FAIL));
                        logger.info(Common.getLogMsg("ConsolidateDeptIncomeSubmit", ApplicationConstants.AUTHENTICATION, ApplicationConstants.FAIL));
                    }
            
            } else {
                request.setAttribute("statuscode", ApplicationConstants.HTTP_STATUS_INVALID_SESSION);
                out.write(new Gson().toJson(ApplicationConstants.HTTP_STATUS_INVALID_SESSION));
                logger.info(Common.getLogMsg("ConsolidateDeptIncomeSubmit", ApplicationConstants.AUTHENTICATION, ApplicationConstants.INVALID_SESSION));
            }
        } catch (Exception ex) {
            ...
            logger.info(Common.getLogMsg("ConsolidateDeptIncomeSubmit", ApplicationConstants.AUTHENTICATION, ApplicationConstants.HTTP_STATUS_EXCEPTION));
        } finally {
            out.close();
        }
    }
